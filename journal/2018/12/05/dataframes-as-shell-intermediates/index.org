...
Of course the idea is not as universal as I might be hinting. There are many
popular tools that return different forms of structured data in formats like
json, xml, sexps etc. but keeping a dataframes structure does make sense for
many of the core unix tools and many data based tools.
...
dataframes, with type constraints and variable naming might have some nicer side
effects.
