...
I have been using them to maintain a lot of /buttons/, or things that are
...
value of ~GITHUB~. Works fine inside Emacs, but html export doesn't. A workaround
is to probably just use a snippet which looks up the property before creating
the link. Since this repeats the url, it hurts portability of links (though I
don't know why I would want to move these links around). A better way might be
to have the snippet prepend a uuid and the export can then pick up the position.
Meh, not that high on priority.
