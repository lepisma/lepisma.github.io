...
external tool. For example, if you use [[https://github.com/generate/generate-license][generate-license]] for licenses, you can
add a block with ~:obtt eval~ in the header.
...
templates. But it should easy to resolve by allowing users to name variables and
recreating the numbered variables that yasnippet expects.
