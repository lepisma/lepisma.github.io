...
It turns out =.kra= files are simple enough to extract the merged png from the
without invoking Krita itself and so the overall experience is much faster now.
Future releases will add support for /creating/ new sketch files from within
Emacs.
