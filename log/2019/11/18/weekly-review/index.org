...
Missed a bunch of these reviews and so will refrain from being elaborate this
time. I am also starting to add a timestamp in every capture from now on so I
can clearly say which things happened in /last/ week and which things earlier.
...
+ I have been thinking about a more programmer friendly API for
  ...
  framework. Looking at the whole process as a sort of probabilistic parsing
  running on audio streams can work out. Plus you, probably, get to design flows
  using something like [[https://en.wikipedia.org/wiki/Parser_combinator][parser combinators]].
