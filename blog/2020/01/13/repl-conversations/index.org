...
This malleability is not a learning[fn::Many researches in self learning systems
are orthogonal here as you can have low generalization and still make a natural
language REPL.] but a design problem at the moment. A problem similar to
designing a programming language. What we want the language user to accomplish
is our focus right now. How we can let the language itself grow is a problem we
will be designing systems for in the future. Once this interaction language is
capable enough, we can have an ecosystem of extensions and libraries making a
...
language in a smooth way". The smoothness here focuses on extendibility in terms
...
Language Understanding, Dialog Management, Text To Speech etc. Improving metrics
under the current framework is a decent and important goal but supporting a
language for others to build on top of, without involving many sublanguages, is
