...
- You create a model instance ~m~, optionally parametrizing it.
#+BEGIN_SRC python
model = Model(params)
#+END_SRC
...
#+END_SRC
- For predicting/testing you call a /predict/ method on ~m~ using the test data.
#+BEGIN_SRC python
model.predict(test_data)
#+END_SRC

In the functional approach on the other hand, the training and testing will go
as functions taking in a /model/ and returning a new /model/, thus inflicting the
update.

One major benefit here is in testing where we get complete control of the
...
might be present telling to choose one function than the other in the state
...
#+BEGIN_SRC lisp
  (defun evolve (data &optional ({} init-{}))
    ...
    (let ((new-{} (do-something data {})))
      (lambda (data &optional (state new-{}))
        (evolve data {})))))
#+END_SRC
...
the center stage which feels weird for the most use cases. But the /state/ feels
more organized in some ways. Probably there will be more to see when I get to
use something with a lot of state based branching.
