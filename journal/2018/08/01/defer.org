...
stack few function calls for later execution. Specifically, just before the
function exists, irrespective of how it actually exited. Something like this is
also available in R in the guise of ~on.exit~ function. Here is an example from
the chapter on functions in [[http://adv-r.had.co.nz/Functions.html#on-exit][Advanced R]]:
