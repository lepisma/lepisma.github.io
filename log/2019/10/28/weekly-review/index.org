...
- Tried making a speech to intent system based on phoneme decoding. An
  interesting result was that dense representation learning for phonemes helped
  a lot compared to even higher order n-grams. My hunch is that a single
  sequence of phoneme transcription is just too brittle.
...
- Explored custom jupyter notebook widgets. The system is really useful and I am
  now trying to move a lot of internal EDA, tagging etc. systems to these.
