...
You have your usual org code block with few specifications (like =NAME=
for variable name to be used for the block result). Essentially this
snip is counting Google search hits for "[name] meme" for different
=names=. It uses /requests/ to get the search results. Now for parsing
the HTML element which says something like "About 100 results found",
lets pass the results to JS
