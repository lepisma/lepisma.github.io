#+TITLE: Dialog /Veneering/
#+SETUPFILE: ../../../../../assets/export.setup
#+TAGS: ml

A lot of things happen in human-machine dialog systems which don't fit in our
default definition of dialogs. There are errors from machine's side and loads of
counter behaviors shown up from the human side. Compared to human-human
conversations, it's harder to look at these conversations from the viewpoint of
earlier works in dialog formalization like (say) [[http://www-bcf.usc.edu/~billmann/dialogue/dmt.htm][Dialog Macrogame Theory]].
...
/buggish/ dialogs can be /veneered/[fn::Using this term since it shows the initial
...
+ ~M~: {{{color-bg(red, okay at 3pm\, what date?)}}}
...
What might be possible is that for /any/ dialog, we can keep the parse
constant[fn::You will need to be someone who knows both parties' mental models.
Here you need to be a human who has worked on the machine.] while moving the
whole conversation around in a certain /coherence-blame space/ which look like
this:

#+begin_aside
You can totally play with context to make dialog A coherent. As an example,
suppose that there is an internal joke connected to our human and 3pm which the
machine is trying to pick on. But for all practical purposes, we can safely
assume the usual common sense understanding of coherence.
#+end_aside
...
that look like /unreasonable errors/ now.] when the coherence goes up as shown
below. Also when parses become richer and/or more correct the horizontal span
gets tighter.

[[file:./space-feasible.svg]]

Another interesting point is that the x axis doesn't really put roles to human
or machine. The dumbest bug is equivalent to a reasonable misunderstanding if we
are allowed to do veneers. Of course this is no relief since nothing got fixed.
Taking this idea to human-human, open conversations, one major challenge is to
identify the underlying parse. Can we really separate utterances from mental
parses here? We sure have /interpretations/ as intermediate representations so we
might try that.
...
2. Experiment with few datapoints to see if it's possible to /always/ make a zero
   blame coherent dialog from any starting point. If yes, can the effort be
   quantified?
3. Formalize terms like /blame/, /parse/ etc.
4. Attempt creating a data set of bad dialogs where the target is to maintain
   same parses (given a sets of NLU systems for both parties) while increasing
